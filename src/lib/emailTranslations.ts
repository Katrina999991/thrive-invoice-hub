// Email template translations (same as edge function)
...
export function translateTemplate(
  englishText: string, 
  templateType: 'newInvoice' | 'overdue' | 'paymentConfirmation',
  isSubject: boolean = false
): string {
  const frenchDefault = isSubject 
    ? emailTranslations.fr[templateType].subject
    : emailTranslations.fr[templateType].body;
  
  const englishDefault = isSubject
    ? emailTranslations.en[templateType].subject
