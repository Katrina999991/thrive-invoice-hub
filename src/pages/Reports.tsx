...
const Reports = () => {
  ...
  const exportNetTaxReportToPDF = async () => {
    ...
    doc.roundedRect(margin, yOffset, pageWidth - (margin * 2), 70, 3, 3, 'F');
    ...
    doc.text(
      `${language === 'fr' ? 'Taxes collectees (ventes)' : 'Collected Taxes (Sales)'}:`,
      margin + 5,
      yOffset + 24
    );
    doc.setFont('helvetica', 'bold');
    ...
    doc.setFont('helvetica', 'normal');
    doc.text(
      `${language === 'fr' ? 'Credits de taxes (depenses)' : 'Tax Credits (Expenses)'}:`,
      margin + 5,
      yOffset + 36
    );
    doc.setFont('helvetica', 'bold');
    ...
    doc.line(margin + 5, yOffset + 42, margin + 140, yOffset + 42);
    
    // Net Amount Payable
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(12);
    doc.text(
      `${language === 'fr' ? 'Montant net a payer' : 'Net Amount Payable'}:`,
      margin + 5,
      yOffset + 52
    );
    doc.setTextColor(59, 130, 246); // Blue
    ...
    doc.text(
      language === 'fr' 
        ? 'Taxes collectees sur les ventes moins les credits de taxes sur les depenses' 
        : 'Collected taxes on sales minus tax credits from expenses',
      margin + 5,
      yOffset + 62
    );
    doc.setTextColor(0, 0, 0);
            ...
            {taxData && (
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>{getReportTranslation('taxesPaidExpensesDesc', language)}</CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex gap-2">
                    <Button 
                      onClick={exportTaxesPaidToPDF} 
                      variant="outline" 
                      size="sm"
                    ...
                    <Button 
                      onClick={exportTaxesPaidToExcel} 
                      variant="outline" 
                      size="sm"
                      disabled={!taxData || taxData.totalExpenseTaxAmount === 0}
                    >
