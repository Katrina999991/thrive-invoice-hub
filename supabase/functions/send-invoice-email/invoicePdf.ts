...
// This module generates Invoice PDFs with IDENTICAL output to
...
function calculateLogoDimensions(
  ...
  try {
    ...
    if (props && props.width && props.height) {
      ...
      const originalWidthPt = props.width * 0.75; // Approximate px to pt conversion
...
export async function generateInvoicePdfForEmail(options: InvoicePdfOptions): Promise<string> {
  const {
    invoice,
    client,
    company,
  ...
  if (template === 'creative') {
    ...
    const docTitle = `${t.title} ${invoice.invoice_number}`;
    doc.setFontSize(12);
  ...
  if (client) {
    ...
    // Invoice number and dates (right side) - not for creative
    if (template !== 'creative') {
      doc.setFontSize(11);
      doc.setFont('helvetica', 'bold');
      if (template === 'professional') {
      ...
      const docTitle = `${t.documentNumber} ${invoice.invoice_number}`;
    ...
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(100, 100, 100);
    const issueDateText = `${t.issueDate}: ${invoice.issue_date}`;
    ...
    if (invoice.due_date) {
      const dueDateText = `${t.secondaryDate}: ${invoice.due_date}`;
      const dueDateWidth = doc.getTextWidth(dueDateText);
      doc.text(dueDateText, pageWidth - dateRightMargin - dueDateWidth, clientInfoY + (template === 'creative' ? 6 : 12) + textYOffset);
    }
  ...
  // Totals
  tableData.push(['', '', `${t.subtotal}:`, `$${invoice.subtotal.toFixed(2)}`]);
  ...
  if (company?.taxes && Array.isArray(company.taxes) && company.taxes.length > 0) {
    ...
    tableData.push(['', '', `${t.tax}:`, `$${invoice.tax_amount.toFixed(2)}`]);
  }
  
  tableData.push(['', '', `${t.total}:`, `$${invoice.total.toFixed(2)}`]);
  ...
  const bodyMessage = isClientFrench 
    ? (company?.invoice_body_message_fr || company?.invoice_body_message_en) 
    : (company?.invoice_body_message_en || company?.invoice_body_message_fr);
  ...
  const footerMessage = isClientFrench 
    ...
    : (company?.invoice_footer_message_en || company?.invoice_footer_message);
